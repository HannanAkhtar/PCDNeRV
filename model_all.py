"""
HNeRV / NeRV model and video dataset.

Copied from the official HNeRV implementation (model_all.py, CVPR 2023).
The network math is UNCHANGED. The only edits relative to upstream are
import-level, so the file runs without video-decoding extras:

  1. removed the unused `pytorchvideo` import,
  2. `decord` is imported lazily, only when `--data_path` points at a video
     file (an image-folder dataset such as data/bunny never needs it).

Architecture recap (relevant to PCD group targets):
  * ``HNeRV.encoder``      — ConvNeXt (HNeRV) or Identity (NeRV w/ positional
                             encoding). Auxiliary: NOT part of the decoded
                             video representation.
  * ``HNeRV.decoder[0]``   — the fully-connected stem: a 1x1 Conv2d that maps
                             the (tiny) embedding to the first feature map.
                             For NeRV-PE the input is 1x1 spatial, so this is
                             literally a fully-connected layer; for HNeRV it is
                             a per-position linear map.
  * ``HNeRV.decoder[1:]``  — the convolutional upsampling NeRV blocks.
  * ``HNeRV.head_layer``   — final 3x3 conv to RGB.
"""
...
class VideoDataSet(Dataset):
    def __init__(self, args):
        if os.path.isfile(args.data_path):
            import decord                      # lazy: only needed for video files
